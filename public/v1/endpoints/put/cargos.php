            ...
            if($payload->categoria == 'administrador')
            {
                if($_SERVER['REQUEST_METHOD'] == 'PUT')
                {
                    if(isset($_GET['id']))
                    {
                        ...
                        $sql = "select * from cargos where id = '$id'"; // para verificar se existe o id informado pelo usuário
                        ...
                        if($result->num_rows > 0)
                        {
                            ...
                            {
                                if(cargos::alterar_cargo($id, $nome) == true)
                                {
                                        $informacoes = cargos::mostrar_alterado_cargo($id);
                                        echo json_encode([
                                                    'status' => 200, 
                            ...
                            else
                            {
                                ...
                                echo json_encode([
                                        'status' => 400,
                                        'api_versao' => API_VERSION,
                                        'mensagem' => 'Nome do cargo não informado',
                                        'data_hora_resposta' => date('d-m-Y H:i:s')], 
                                        JSON_PRETTY_PRINT);
                                    exit;
                            }
