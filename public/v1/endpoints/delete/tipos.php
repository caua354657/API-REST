                ...
                if($payload->categoria == 'administrador')
                {
                    if($_SERVER['REQUEST_METHOD'] == 'DELETE')
                    {
                        if(isset($_GET['id']))
                        {
                            ...
                            $sql = "select * from empresas where tipo_cadastro = '$id'";
                            $result = $conexao->query($sql);
                            $linha = $result->fetch_assoc();

                            if($result->num_rows == 0)
                            {
                                if(tipos::excluir_tipos($id) == true)
                                {
                                    echo json_encode([
                                                'status' => 200, 
                            ...
                            else
                            {
                                ...
                                echo json_encode([
                                            "status" => 409,
                                            'api_versao' => API_VERSION,
                                            "mensagem" => "Já existe uma empresa associada a esse tipo. Não é possível excluir.",
                                            'data_hora_resposta' => date('d-m-Y H:i:s'),],
                                            JSON_PRETTY_PRINT
                                        );
                            }
                        ...
                        else
                        {
                            ...
                            echo json_encode([
                                            "status" => 400,
                                            'api_versao' => API_VERSION,
                                            "mensagem" => "ID tipo_cadastro indefinido",
                                            'data_hora_resposta' => date('d-m-Y H:i:s'),],
                                            JSON_PRETTY_PRINT
                                        );
                        }
